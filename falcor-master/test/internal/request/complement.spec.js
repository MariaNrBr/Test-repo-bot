...
describe("findPartialIntersections", () => {
    ...
    describe("prototype pollution prevention", () => {
        ...
        it("prevents constructor pollution", () => {
            const requestedPath = ["constructor", "prototype", "polluted"];
            const optimizedPath = ["constructor", "prototype", "polluted"];
            const pathTree = { };
        ...
        it("prevents pollution via range keys", () => {
            const requestedPath = ["videos", ["__proto__", 1], "title"];
            const optimizedPath = ["videos", ["__proto__", 1], "title"];
            const pathTree = { videos: { 1: { title: null } } };

            expect(findPartialIntersections(requestedPath, optimizedPath, pathTree)).toEqual([
                [["videos", 1, "title"]],
                [["videos", "__proto__", "title"]],
                [["videos", "__proto__", "title"]]
            ]);
        });
    });
